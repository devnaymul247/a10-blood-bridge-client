...
export default function AdminProfileEditor({ admin }) {
  const [isEditing, setIsEditing] = useState(false);
  const [isSaving, setIsSaving] = useState(false);

  const [fullName, setFullName] = useState(admin?.name || '');
  const [bloodGroup, setBloodGroup] = useState(admin?.bloodGroup || '');
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Pre-select admin's saved district so upazila list is ready in edit mode
        if (admin?.district) {
          const matched = districtsList.find((d) => d.name === admin.district);
          if (matched) {
        ...
        if (admin?.upazila) {
          setSelectedUpazilaName(admin.upazila);
        }
      } catch (error) {
  ...
  const isEligible = admin?.isEligible ?? true;
  ...
  const handleCancel = () => {
    ...
    if (admin?.district) {
      const matched = districts.find((d) => d.name === admin.district);
      if (matched) {
        setSelectedDistrict(String(matched.id));
        setSelectedDistrictName(matched.name);
      }
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Manage your personal information and admin credentials.
            </p>
          </div>
          {isEditing ? (
          ...
          <div className="relative px-6">
            <div className="flex items-end gap-4 -mt-6">
              <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-full border-4 border-white bg-white shadow-md overflow-hidden">
                <Image
                  width={96} height={96}
                  src={admin?.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(fullName || 'admin')}&background=8FA1B9&color=000&bold=true`}
                  alt="Avatar"
              ...
              <div className="flex flex-1 flex-col gap-1 pb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <h2 className="text-2xl font-extrabold text-black">{fullName || 'admin'}</h2>
            ...
            <div className="flex flex-col gap-8">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <TextField isDisabled value={admin?.email || ''}>
                ...
                {isEditing ? (
                  ...
                  >
                    ...
                    <Select.Popover className="border border-slate-200 bg-white">
                      <ListBox>
                        {BLOOD_GROUPS.map((g) => (
                          <ListBox.Item key={g.value} id={g.value} textValue={g.label} className="hover:bg-red-50 text-black">
                            <div className="flex items-center gap-2">
